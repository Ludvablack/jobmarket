...
<head>
    ...
    <script>
        function delayStart(form) {
            document.getElementById("loadingStart").style.display = "block";
...
<body>
    ...
    // Pokud zakázka už byla spuštěna → přesměruj
    if (!empty($row['start_event'])) {
        header("Location: assigned_event5.php");
        exit;
    }

    // Kontrola předčasného spuštění
    $currentTime = new DateTime();
    $beginTime = new DateTime($row['begin_event']);

    if ($currentTime < $beginTime) {
        echo "<h2 style='color:red;'>Zakázku není možné provést předčasně.</h2>";
        exit;
    }
    // Kontrola předčasného spuštění
    ...
    // Vrátíme kurzor zpět, aby šlo znovu číst celý výsledek
    $result->data_seek(0);
    ...
    if ($result->num_rows > 0) {
        echo "<table border='1'>";
        echo "<tr><th>ID zakázky</th><th>ID práce</th><th>Název práce</th><th>Hodinová sazba</th><th>Předpokládaný začátek</th><th>Počet hodin</th><th>Výdělek včetně daně</th><th>Start</th></tr>";
        echo "<tbody>";
    ...
    <div style="display:flex; gap:100px; align-items:flex-start; margin-top:20px;">
        ...
        <!-- Formulář START -->
        <br><br>
        <form action="assigned_event3.php" method="POST" onsubmit="return delayStart(this);" style="margin-top:20px;">
        ...
        ">
                ✔ Potvrdit start zakázky
            </button>
        </form>

        <!-- Formulář POZNÁMKA -->
        <br><br>
        <form action="assigned_event4.php" method="POST" onsubmit="return delayNote(this);" style="margin-top:20px;">
            <input type="hidden" name="id_event" value="<?php echo $uzivatel_id; ?>">
        ...
        ">
            ...
            <br><br>
            <label for="comment_note"><strong>Poznámka k zakázce:</strong></label><br>
            <textarea name="comment_event" id="comment_event" rows="3" cols="80"
    ...
    <!-- Hlášky -->
    <div id="loadingStart" style="display:none; font-weight:bold; color:#d32f2f; margin-top:20px;">
        Probíhá start zakázky…
    </div>
